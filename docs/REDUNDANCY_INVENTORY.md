...
done (PR #10, deployed); R10 done (`81b2962`).** The screen table below is the state *before* the consolidation.
...
| **R9** | Server, one-definition rule: `/api/sleep`'s overnight blood oxygen and breathing are computed by their own query (`read/sleep_page.py`), not read from the canonical `spo2_overnight` / `respiratory_rate_sleep` (`derive/hrv_spo2_resp.py`). They agree today (99 vs 99.0) by coincidence of method, not by construction. | **Done:** PR #10 (`7fc9c99`), deployed 2026-09-23. `/api/sleep` reads `spo2_overnight` / `spo2_overnight_min` / `respiratory_rate_sleep` through `derived_night_pivot`; the raw join keeps only skin temperature; no fallback for an underived night. Pre-deploy check: 30 nights, 17 without a derived SpO2 row, 0 of those with SpO2 samples. Owner's values unchanged after deploy (no artifacts in the window). |
