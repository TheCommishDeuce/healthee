...
| **R9** | Server, one-definition rule: `/api/sleep`'s overnight blood oxygen and breathing are computed by their own query (`read/sleep_page.py`), not read from the canonical `spo2_overnight` / `respiratory_rate_sleep` (`derive/hrv_spo2_resp.py`). They agree today (99 vs 99.0) by coincidence of method, not by construction. | **Server change, not started.** Plan: add `spo2_overnight` → `spo2_avg`, `spo2_overnight_min` → `spo2_min`, `respiratory_rate_sleep` → `respiratory_rate` to `_SLEEP_PAGE_METRICS` / `_apply_derived` (the same pivot that already supplies `hrv_sleep_avg` and `rhr`), and keep only `skin_temp_c` in `_PHYSIOLOGY_SQL` (no derived metric exists for it). Touches `tests/read/test_sleep_physiology.py`, the contract seeds and likely the `sleep.json` snapshot, and the app's `sampled` reading kind for those two fields (`sleep_night.dart`) — its absence wording would become the derived one. Own branch + PR, server suite in UTC and Asia/Kolkata. |
| **R10** | Code, found while fixing the Recovery baseline row: three `hoursMinutes` copies that disagreed — Today's sleep card wrote `7h 0m` where Sleep wrote `7h 00m`, and could write `0h 60m`. | **Done:** one `hoursMinutes` in `shared/format/time_labels.dart`; Recovery's minute readings (baseline row, population floor) use it. `durationLabel` (`48m` under an hour, workouts) is a different format and stays. |
